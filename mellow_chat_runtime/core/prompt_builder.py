...
def build_system_prompt(
    ...
    return (
        ...
        f'{tone}\n\n'
        'RP Input Rules:\n'
        '- The user input may contain narration and dialogue\n'
        '- Treat narration as scene and action context\n'
        "- Treat quoted text as the user's spoken line\n"
        '- Respond as an in-scene reaction, not as an explanation\n\n'
        'Output Format:\n'
        '1. One short narration or action paragraph\n'
        '2. One spoken line in quotes\n\n'
        'Rules:\n'
        '- Stay in character\n'
        '- Keep responses coherent with provided world data\n'
        '- Prioritize action plus spoken dialogue over explanation\n'
        ...
        'Output Constraint:\n'
        '- Return only the final in-character RP answer to the user\n'
        '- Do not explain the response or mention instructions\n'
        '- Do not output analysis, planning, checklist, role tags, or transcript markup\n'
        '- Do not emit tokens such as <|im_start|>, <|im_end|>, <|endoftext|>, or <think>\n'
        '- Do not use assistant-style meta language\n'
...
def build_user_prompt(
    ...
    if scene_event is not None:
        parts.append(
            'Parsed User Scene Event:\n'
            f'Raw Input: {scene_event.raw_text or user_text}\n'
            f'User Narration: {scene_event.user_narration or "(none)"}\n'
            f'User Dialogue: {scene_event.user_dialogue or "(none)"}\n'
            f'Input Mode: {scene_event.input_mode}\n'
            f'Target Hint: {target_character_hint or scene_event.target_character_hint or "(none)"}'
        )
